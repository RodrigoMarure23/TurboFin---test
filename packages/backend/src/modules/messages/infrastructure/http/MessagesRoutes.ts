...
messageRouter.get("/thread/:messageId", (req, res) =>
  getMessageThreadController.run(req, res),
);

// --- RUTAS PROTEGIDAS ---

// Obtener actividad reciente del usuario (NUEVO)
messageRouter.get("/activity", authMiddleware, (req, res) =>
  getRecentActivityController.run(req, res),
);

messageRouter.post("/", authMiddleware, (req, res) =>
  postMessageController.run(req, res),
);

messageRouter.post("/reply", authMiddleware, (req, res) =>
  postReplyController.run(req, res),
);

messageRouter.post("/:messageId/like", authMiddleware, (req, res) =>
  postLikeController.run(req, res),
);
